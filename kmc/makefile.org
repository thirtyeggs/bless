...
#CC 	= g++
# BLESS
CFLAGS	= -Wall -O3 -m64 -fopenmp -Wl,--whole-archive -pthread -Wl,--no-whole-archive -std=c++11 
# BLESS
CLINK	= -lm -fopenmp -O3 -Wl,--whole-archive -pthread -Wl,--no-whole-archive -std=c++11 
...
kmc: $(KMC_OBJS)
	-mkdir -p $(KMC_BIN_DIR)
	$(CC) -o $(KMC_BIN_DIR)/$@ $^ $(KMC_LIBS) $(CLINK)
kmc_dump: $(KMC_DUMP_OBJS)
	-mkdir -p $(KMC_BIN_DIR)
	$(CC) -o $(KMC_BIN_DIR)/$@ $^ $(CLINK)
clean:
	-rm $(KMC_MAIN_DIR)/*.o
	-rm $(KMC_API_DIR)/*.o
	-rm $(KMC_DUMP_DIR)/*.o
